...
// libraries for mod support
// these are only the linux libraries
// binaries for mod support are Copyright (C) 2005-2021 Tecgraf/PUC-Rio and the Kepler Project
extern "C"
{
...
int main() {
    ...
    // neccessary variables for mod support declaration
	int intModPrompt;
	int intModCombatOption;
	int intModEnemyRNG;
	int intModPlayerStateVariable;
	int intModInput;
    
    // opening lua VM with neccessary settings for mod support
    ...
    // extra line for mod
    printf("\n");
    //mod prompt: 1 = enable mod support, 2 = disable mod support
    intModPrompt = 2;
    

    // asking player for what mod file to use
    if (intModPrompt == 1) {
		
		// Tag for Mod
		printf("Mod:\n");
        // check if its going to run
        if (luaL_dofile(L, "The_Game_Of_Trusters_Mod.lua")){
            
            // notice to player that mod file could not be loaded
            cerr << "Error loading mod file: " << lua_tostring(L, -1) << endl;
            lua_close(L); exit(0);
            
        }
        
        
        // calls Mod Notice and Name string
        lua_getglobal(L, "strNoticeAndName");
        if (lua_isstring(L, -1)) {
			
			// gets the mod notice and name
			string strModNoticeAndName = (string)lua_tostring(L, -1);
			// prints Mods notice and name
			cout << strModNoticeAndName << endl;
    ...
    else if (intModPrompt == 2) {
        void enemy();
        printf("You have decided not to use a mod file. \n");
    }
    ...
    int i = 5;
    // mod support functionality
    if (intModPrompt == 1) {

    // while loop needed for game
    while (i == 5) {
        intPlayerStateVariable = rand() % 8 + 1;
        intModPlayerStateVariable = rand() % 2 + 1;
        intModEnemyRNG = rand() % 2 + 1;
        switch (intPlayerStateVariable) {
            ...
            case 6:
                ...
                break;
            case 7:
				// if statements for dermining enemy
				if (intModEnemyRNG == 1) {
					// calls Mod Enemy 1 string
					lua_getglobal(L, "strEnemy1");
					if (lua_isstring(L, -1)) {
						
						// gets the mod enemy 1 string
						string strModEnemy1 = (string)lua_tostring(L, -1);
						// prints Mods enemy 1 string
						cout << strModEnemy1 << endl;
						// Mod input
						cin >> intModInput;
						// removes string from stack
						lua_pop(L, 1);
						
					}
					else if (!lua_isstring(L, -1)) {
						
						printf("strEnemy1 is not a string");
						lua_close(L); exit(0);
					...
					if (intModInput == 0){
						...
						// calls Mod Dice int
						lua_getglobal(L, "intDice");
						if (lua_isnumber(L, -1)) {
							
							// gets the mod dice int
							int intModDice = (int)lua_tonumber(L, -1);
							// logic for combat
							if (intModDice == 1) {
								printf("You have successfully defeated the enemy\n");
								// removes int from stack
								lua_pop(L, 1);
							}
							else if (intModDice == 2) {
								printf("Game Over\n");
								lua_close(L); exit(0);
							}
						...
						printf("intDice is not a number\n");
						lua_close(L); exit(0);
						
						}
						
					}
					else if (intModInput == 1){
						...
						if (lua_isnumber(L, -1)) {
							...
							// logic for combat
							if (intModDice == 1) {
								printf("You have successfully talked to the enemy\n");
								// removes int from stack
								lua_pop(L, 1);
							}
							else if (intModDice == 2) {
				...
				else if (intModEnemyRNG == 2) {
					
					// calls Mod Enemy 2 string
					lua_getglobal(L, "strEnemy2");
					if (lua_isstring(L, -1)) {
						
						// gets the mod enemy 2 string
						string strModEnemy2 = (string)lua_tostring(L, -1);
						// prints Mods enemy 2 string
						cout << strModEnemy2 << endl;
						// Mod input
						cin >> intModInput;
						// removes string from stack
						lua_pop(L, 1);
						...
						printf("strEnemy2 is not a string\n");
						lua_close(L); exit(0);
					...
					if (intModInput == 0){
						...
						if (lua_isnumber(L, -1)) {
							...
							// logic for combat
							if (intModDice == 1) {
								printf("You have successfully the defeated the enemy\n");
								// removes int from stack
								lua_pop(L, 1);
							}
							else if (intModDice == 2) {
                ...
                break;
            case 8:
				// if statements for dermining player state
                if (intModPlayerStateVariable == 1) {
					
					// calls Mod Player Location State 1 string
					lua_getglobal(L, "strPlayerLocationState1");
					if (lua_isstring(L, -1)) {
						
						// gets the mod Player Location State 1
						string strModPlayerLocationState1 = (string)lua_tostring(L, -1);
						// prints Mods Player Location State 1
						cout << strModPlayerLocationState1 << endl;
						// Mod input
						cin >> intModInput;
						// removes string from stack
						lua_pop(L, 1);
						...
						printf("strPlayerLocationstate1 is not a string\n");
						lua_close(L); exit(0);
					...
					// if statements
					if (intModInput == 1){
						// Telling player you went left
						printf("You went left\n");
					}
					else if (intModInput == 2){
						// Telling player you went right
				...
				else if (intModPlayerStateVariable == 2) {
					
					// calls Mod Player Location State 2 string
					lua_getglobal(L, "strPlayerLocationState2");
					if (lua_isstring(L, -1)) {
						
						// gets the mod Player Location State 2
						string strModPlayerLocationState2 = (string)lua_tostring(L, -1);
						// prints Mods Player Location State 2
						cout << strModPlayerLocationState2 << endl;
						// Mod input
						cin >> intModInput;
						// removes string from stack
						lua_pop(L, 1);
						...
						printf("strPlayerLocationState2 is not a string\n");
						lua_close(L); exit(0);
